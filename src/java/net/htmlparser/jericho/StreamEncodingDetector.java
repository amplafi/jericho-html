...
final class StreamEncodingDetector {
	...
	public Reader openReader() throws UnsupportedEncodingException {
		if (encoding==null) {
            return new InputStreamReader(inputStream,DEFAULT_8BIT); // encoding==null only if input stream is empty so use an arbitrary encoding.
        }
		if (!EncodingDetector.isEncodingSupported(encoding)) {
	...
	private boolean init() throws IOException {
		...
		if (b1==0xEF) {
			if (b2==0xBB && b3==0xBF) {
                return setEncoding(UTF_8,"UTF-8 Byte Order Mark (EF BB BF)",3,b4==-1);
            }
		} else if (b1==0xFE) {
 			if (b2==0xFF) {
                return setEncoding(UTF_16,"UTF-16 big-endian Byte Order Mark (FE FF)",2,b3==-1);
            }
		} else if (b1==0xFF) {
 			if (b2==0xFE) {
 				if (b3==0 && b4==0) {
                    return setEncoding(UTF_32,"UTF-32 little-endian Byte Order Mark (FF EE 00 00)",4,b5==-1);
                }
				return setEncoding(UTF_16,"UTF-16 little-endian Byte Order Mark (FF EE)",2,b3==-1);
			}
		} else if (b1==0) {
 			if (b2==0 && b3==0xFE && b4==0xFF) {
                return setEncoding(UTF_32,"UTF-32 big-endian Byte Order Mark (00 00 FE FF)",4,b5==-1);
            }
		} else if (b1==0x0E) {
 			if (b2==0xFE && b3==0xFF) {
                return setEncoding(SCSU,"SCSU Byte Order Mark (0E FE FF)",3,b4==-1);
            }
		} else if (b1==0x2B) {
 			if (b2==0x2F && b3==0x76) {
                return setEncoding(UTF_7,"UTF-7 Byte Order Mark (2B 2F 76)",3,b4==-1);
            }
		} else if (b1==0xDD) {
 			if (b2==0x73 && b3==0x66 && b4==0x73) {
                return setEncoding(UTF_EBCDIC,"UTF-EBCDIC Byte Order Mark (DD 73 66 73)",4,b5==-1);
            }
		} else if (b1==0xFB) {
 			if (b2==0xEE && b3==0x28) {
                return setEncoding(BOCU_1,"BOCU-1 Byte Order Mark (FB EE 28)",3,b4==-1);
            }
		}
		...
		if (b4==-1) {
			...
			if (b2==-1 || b3!=-1) {
                return setEncoding(DEFAULT_8BIT,"default 8-bit ASCII-compatible encoding (stream 3 bytes long)",0,false); // The stream contains exactly 1 or 3 bytes, so assume an 8-bit encoding regardless of whether any 00 bytes are present.
            }
			// The stream contains exactly 2 bytes.
			if (b1==0) {
                return setEncoding(UTF_16BE,"default 16-bit BE encoding (byte stream starts with 00, stream 2 bytes long)",0,false);
            }
			if (b2==0) {
                return setEncoding(UTF_16LE,"default 16-bit LE encoding (byte stream pattern XX 00, stream 2 bytes long)",0,false);
            }
		...
		if (b1==0) {
			// pattern 0???
			if (b2==0) {
                return setEncoding(UTF_32BE,"default 32-bit BE encoding (byte stream starts with 00 00)",0,false); // pattern 00?? most likely indicates UTF-32BE
            }
			// pattern 0X??
		...
		if (b4==0) {
			// pattern X??0
			if (b3==0) {
                return setEncoding(UTF_32LE,"default 32-bit LE encoding (byte stream starts with pattern XX ?? 00 00)",0,false); // pattern X?00 most likely indicates UTF-32LE
            }
			// pattern X?X0
		...
		// pattern XX?X
		if (b3==0) {
            return setEncoding(UTF_16BE,"default 16-bit BE encoding (byte stream starts with pattern XX XX 00 XX)",0,false); // pattern XX0X likely to indicate a 16-bit BE encoding with the first character > U+00FF.
        }
		// pattern XXXX
		...
		if (b1==0x4C) { // first character is EBCDIC '<' (ASCII 'L'), check a couple more characters before assuming EBCDIC encoding:
			if (b2==0x6F && b3==0xA7 && b4==0x94) {
                return setEncoding(EBCDIC,"default EBCDIC encoding (<?xml...> detected)",0,false); // first four bytes are "<?xm" in EBCDIC ("Lo" in Windows-1252)
            }
			if (b2==0x5A && b3==0xC4 && b4==0xD6) {
                return setEncoding(EBCDIC,"default EBCDIC encoding (<!DOCTYPE...> detected)",0,false); // first four bytes are "<!DO" in EBCDIC ("LZ" in Windows-1252)
            }
			if ((b2&b3&b4&0x80)!=0) {
                return setEncoding(EBCDIC,"default EBCDIC-compatible encoding (HTML element detected)",0,false); // all of the 3 bytes after the '<' have the high-order bit set, indicating EBCDIC letters such as "<HTM" ("L" in Windows-1252), or "<htm" ("L" in Windows-1252)
            }
			if (b2==0x6F && b3==0xA7 && b4==0x94) {
                return setEncoding(EBCDIC,"default EBCDIC encoding (<?xml...> detected)",0,false); // first four bytes are "<?xm" in EBCDIC ("Lo" in Windows-1252)
            }
