...
public class TextExtractor implements CharStreamSource {
	...
	private static Map map=initDefaultAttributeIncludeCheckerMap(); // maps each possibly included attribute name to an AttributeIncludeChecker instance
				...
				if (nodeSegment instanceof Tag) {
					...
					if (tag.getTagType()==StartTagType.NORMAL) {
						...
						if (includeAttributes) {
							for (Iterator attributeIterator=startTag.getAttributes().iterator(); attributeIterator.hasNext();) {
