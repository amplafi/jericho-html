...
import au.id.jericho.lib.html.nodoc.*;
import java.util.*;
import java.io.*;
 ...
 * <p>
 * As of version 2.4, any {@linkplain TagType#isServerTag() server tags} encountered inside the attributes area of a non-server tag
...
public final class Attributes extends SequentialListSegment {
	...
	private Attributes(final Source source, final int begin, final int end, final LinkedList attributeList, final boolean containsServerTagOutsideOfAttributeValue) {
			...
			while (!isTerminatingCharacter) {
				...
				if (ch=='<') {
					...
					if (interlopingTag!=null) {
						...
						if (state!=IN_VALUE) containsServerTagOutsideOfAttributeValue=true;
						continue;
					}
				}
