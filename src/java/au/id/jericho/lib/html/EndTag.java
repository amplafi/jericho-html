...
public final class EndTag extends Tag {
	 ...
	 * <p>
	 * Called from {@link Source#findPreviousEndTag(int pos, String name)}.
	 *
	 * @param source  the {@link Source} document.
	 ...
	 */
	static EndTag findPrevious(final Source source, final int pos, final String name, final EndTagType endTagType) {
		if (name==null) return (EndTag)Tag.findPreviousTag(source,pos,endTagType);
	 ...
	 * <p>
	 * Called from {@link Source#findNextEndTag(int pos, String name, EndTagType endTagType)}.
	 *
	 * @param source  the {@link Source} document.
	 * @param pos  the position to search from.
	 * @param name  the {@linkplain #getName() name} of the tag including its {@linkplain TagType#getNamePrefix() prefix} (must be lower case, may be null).
	 ...
	 */
	static EndTag findNext(final Source source, final int pos, final String name, final EndTagType endTagType) {
		if (name==null) return (EndTag)Tag.findNextTag(source,pos,endTagType);
		if (name.length()==0) throw new IllegalArgumentException("name argument must not be zero length");
		final String searchString=endTagType.START_DELIMITER_PREFIX+name;
		try {
			final ParseText parseText=source.getParseText();
	...
	static EndTag findPrevious(final Source source, int pos) {
		while (true) {
	...
	static EndTag findNext(final Source source, int pos) {
		while (true) {
