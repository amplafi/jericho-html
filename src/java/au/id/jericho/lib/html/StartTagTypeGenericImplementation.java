...
public class StartTagTypeGenericImplementation extends StartTagType {
	 ...
	 *   {@linkplain #getNamePrefix() name prefix} using the {@link Source#findNameEnd(int pos)} method.
	 ...
	 *  <li>
	 *   Find the {@linkplain Tag#getEnd() end} of the tag using the {@link #findEnd(Source, int pos)} method,
	...
	protected Tag constructTagAt(final Source source, final int pos) {
		...
		if (isNameAfterPrefixRequired()) {
			final int extendedNameEnd=source.findNameEnd(nameEnd);
		...
		if (hasAttributes()) {
			...
			end=findEnd(source,attributes.getEnd()); // should always return a valid end
		} else {
			end=findEnd(source,nameEnd);
			if (end==-1) {
				if (source.logger.isInfoEnabled()) source.logger.info(source.getRowColumnVector(pos).appendTo(new StringBuffer(200).append("StartTag ").append(name).append(" at ")).append(" not recognised as type '").append(getDescription()).append("' because it has no closing delimiter").toString());
				return null;
			}
		}
	...
	/**
	 * Finds the {@linkplain Tag#getEnd() end} of a tag of this type, starting from the specified position in the specified source document.
	 * <br />(<a href="TagType.html#ImplementationAssistance">implementation assistance</a> method)
	 * <p>
	 ...
	 */
	protected int findEnd(final Source source, final int pos) {
