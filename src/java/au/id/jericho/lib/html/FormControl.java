...
public abstract class FormControl extends Segment {
	...
	static FormControl construct(final Element element) {
		...
		if (tagName==Tag.INPUT) {
			...
			if (formControlType==null) {
				if (element.source.logger.isInfoEnabled()) element.source.logger.info(element.source.getRowColumnVector(element.begin).appendTo(new StringBuffer(200)).append(": INPUT control with unrecognised type \"").append(typeAttributeValue).append("\" assumed to be type \"text\"").toString());
			...
			if (formControlType==FormControlType.NON_FORM_CONTROL) return null;
	...
	private static void findAll(final Segment segment, final ArrayList list, final String tagName) {
		for (final Iterator i=segment.findAllElements(tagName).iterator(); i.hasNext();) {
			final FormControl formControl=((Element)i.next()).getFormControl();
	...
	private void verifyName() {
		...
		if (source.logger.isInfoEnabled()) source.logger.info(getElement().source.getRowColumnVector(getElement().begin).appendTo(new StringBuffer(200)).append(": compulsory \"name\" attribute of ").append(formControlType).append(" control is ").append(missingOrBlank).toString());
	}
