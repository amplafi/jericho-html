...
final class EndTagTypeUnregistered extends EndTagType {
	...
	protected Tag constructTagAt(final Source source, final int pos) {
		...
		if (source.logger.isInfoEnabled()) source.logger.info(source.getRowColumnVector(pos).appendTo(new StringBuffer(200).append("Encountered possible EndTag at ")).append(" whose content does not match a registered EndTagType").toString());
		return endTag;
	}
}
