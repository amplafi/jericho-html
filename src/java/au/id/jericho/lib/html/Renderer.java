...
public final class Renderer implements CharStreamSource {
	...
	public void writeTo(final Writer writer) throws IOException {
		new Processor(rootSegment,getMaxLineLength(),getNewLine(),getIncludeHyperlinkURLs(),getDecorateFontStyles(),getConvertNonBreakingSpaces(),getBlockIndentSize(),getListIndentSize(),getListBullets(),getTableCellSeparator()).writeTo(writer);
	}
	 ...
	 * <p>
	 * When this property is <code>true</code>, the URL of each hyperlink is included in the output, enclosed in angle brackets, after the hyperlink label.
	 * <p>
	 * <dl>
	 *  <dt>Example:</dt>
	 *  <dd>
	 *   <p>
	 *   When this property is <code>true</code>, the following HTML:
	...
	private static final class Processor {
		...
		public Processor(final Segment rootSegment, final int maxLineLength, final String newLine, final boolean includeHyperlinkURLs, final boolean decorateFontStyles, final boolean convertNonBreakingSpaces, final int blockIndentSize, final int listIndentSize, final char[] listBullets, final String tableCellSeparator) {
		...
		private void writeSegment(final Segment segment) throws IOException {
			writeSegmentProcessingChildElements(segment.begin,segment.end,segment.getChildElements());
		}
	
		private void writeElementContent(final Element element) throws IOException {
			...
			writeSegmentProcessingChildElements(Math.max(renderedIndex,contentBegin),contentEnd,element.getChildElements());
		}
	
		private void writeSegmentProcessingChildElements(final int begin, final int end, final List childElements) throws IOException {
			int index=begin;
			for (Iterator i=childElements.iterator(); i.hasNext();) {
				Element childElement=(Element)i.next();
				if (index>=childElement.end) continue;
				if (index<childElement.begin) writeSegmentRemovingTags(index,childElement.begin);
				getElementHandler(childElement).process(this,childElement);
				index=Math.max(renderedIndex,childElement.end);
			}
			if (index<end) writeSegmentRemovingTags(index,end);
		}
		...
		private void writeSegmentRemovingTags(final int begin, final int end) throws IOException {
			int index=begin;
			while (true) {
				Tag tag=source.findNextTag(index);
				if (tag==null || tag.begin>=end) break;
				writeSegment(index,tag.begin);
				index=tag.end;
			}
			writeSegment(index,end);
		}

		private void writeSegment(int begin, final int end) throws IOException {
			// assert begin<end
			if (begin<renderedIndex) begin=renderedIndex;
			if (begin>=end) return;
			try {
				if (preformatted)
					writePreformattedSegment(begin,end);
				else
					writeNonPreformattedSegment(begin,end);
			} finally {
				if (renderedIndex<end) renderedIndex=end;
			}
		}
	
		private void writePreformattedSegment(final int begin, final int end) throws IOException {
			// assert begin<end, begin>=renderedIndex
			if (isStartOfBlock()) writeBlockVerticalMargin();
		...
		private void writeNonPreformattedSegment(final int begin, final int end) throws IOException {
			// assert begin<end, begin>=renderedIndex
			...
			while (true) {
				...
				if (col+i-textIndex+1>=maxLineLength) {
					if (lastCharWhiteSpace && (blockIndentLevel|listIndentLevel)==0) write(' ');
					startNewLine(0);
				} else if (lastCharWhiteSpace) {
					write(' ');
				}
				write(text,textIndex,i);
		...
		private void startNewLine(int verticalMargin) throws IOException {
			...
			for (int i=0; i<requiredNewLines; i++) writer.write(newLine);
			atStartOfLine=true;
			lastCharWhiteSpace=ignoreInitialWhitespace=false;
		}
		...
		private void writeIndent() throws IOException {
			for (int i=blockIndentLevel*blockIndentSize; i>0; i--) writer.write(' ');
			if (bullet) {
				for (int i=(listIndentLevel-1)*listIndentSize; i>0; i--) writer.write(' ');
				if (listBulletNumber==UNORDERED_LIST) {
					for (int i=listIndentSize-2; i>0; i--) writer.write(' ');
					writer.write(listBullets[(listIndentLevel-1)%listBullets.length]);
					writer.write(' ');
				} else {
					String bulletNumberString=Integer.toString(listBulletNumber);
					for (int i=listIndentSize-bulletNumberString.length()-2; i>0; i--) writer.write(' ');
					writer.write(bulletNumberString);
					writer.write(". ");
				}
				bullet=false;
			} else {
				for (int i=listIndentLevel*listIndentSize; i>0; i--) writer.write(' ');
			}
			col=blockIndentLevel*blockIndentSize+listIndentLevel*listIndentSize;
			atStartOfLine=false;
		}
	
		private Processor write(final char ch) throws IOException {
			if (atStartOfLine) writeIndent();
			writer.write(ch);
			col++;
			return this;
		}
		
		private Processor write(final String text) throws IOException {
			if (atStartOfLine) writeIndent();
			writer.write(text);
			col+=text.length();
			return this;
		...
		private void write(final CharSequence text, final int begin, final int end) throws IOException {
			if (atStartOfLine) writeIndent();
			for (int i=begin; i<end; i++) writer.write(text.charAt(i));
			col+=end-begin;
		}
		...
		private static class FontStyleElementHandler implements ElementHandler {
			...
			public void process(Processor x, Element element) throws IOException {
				if (x.decorateFontStyles) {
					...
					x.write(decorationChar);
					x.writeElementContent(element);
					if (x.decorateFontStyles) x.write(decorationChar);
				} else {
					x.writeElementContent(element);
				}
			}
		...
		private static class A_ElementHandler implements ElementHandler {
			...
			public void process(Processor x, Element element) throws IOException {
				...
				if (!x.includeHyperlinkURLs || href==null || href.equals("#") || href.startsWith("javascript:")) return;
		...
		private static class TD_ElementHandler implements ElementHandler {
			...
			public void process(Processor x, Element element) throws IOException {
				if (!x.isStartOfBlock()) x.write(x.tableCellSeparator);
				x.lastCharWhiteSpace=false;
				x.writeElementContent(element);
			}
		}
