...
public final class StartTag extends Tag {
		...
		if (element==Element.NOT_CACHED) {
			...
			if (endTag!=null) {
				if (endTag.element!=Element.NOT_CACHED) {
					...
					if (source.logger.isInfoEnabled() && !element.equals(endTag.element)) source.logger.info(source.getRowColumnVector(endTag.begin).appendTo(new StringBuffer(200).append("End tag ").append(endTag).append(" at ")).append(" terminates more than one element").toString()); 
				}
	...
	StringBuffer appendDebugTag(final StringBuffer sb) {
	...
	StringBuffer appendDebugTagType(final StringBuffer sb) {
	...
	private EndTag findEndTagInternal() {
		...
		final EndTag nextEndTag=source.findNextEndTag(end,endTagType.getEndTagName(name),endTagType);
		if (nextEndTag!=null) {
			if (startTagType==StartTagType.NORMAL && HTMLElements.END_TAG_REQUIRED_NESTING_FORBIDDEN_SET.contains(name)) {
				...
				if (source.logger.isInfoEnabled()) source.logger.info(source.getRowColumnVector(begin).appendTo(new StringBuffer(200).append("StartTag at ")).append(" missing required end tag - invalid nested start tag encountered before end tag").toString());
			...
			final Segment[] findResult=findEndTag(nextEndTag,checkForEmptyElementTag,Tag.isXMLName(name));
			if (findResult!=null) return (EndTag)findResult[0];
		}
		if (source.logger.isInfoEnabled()) source.logger.info(source.getRowColumnVector(begin).appendTo(new StringBuffer(200).append("StartTag at ")).append(" missing required end tag").toString());
		return null;
	}
	...
	private EndTag findOptionalEndTag(final HTMLElementTerminatingTagNameSets terminatingTagNameSets) {
		...
		while (pos<source.end) {
			final Tag tag=Tag.findNextTag(source,pos);
			if (tag==null) break;
	...
	static StartTag findPrevious(final Source source, final int pos, final String searchName, final StartTagType searchStartTagType) {
		return findPrevious(source,pos,searchName,searchStartTagType,searchStartTagType==StartTagType.NORMAL ? Tag.isXMLName(searchName) : true);
	}

	static StartTag findPrevious(final Source source, final int pos, final String searchName, final StartTagType searchStartTagType, final boolean isXMLTagName) {
		// searchName is already in lower case
		if (searchName==null) return (StartTag)source.findPreviousTag(pos,searchStartTagType);
	...
	static StartTag findNext(final Source source, final int pos, final String searchName, final StartTagType searchStartTagType) {
		return findNext(source,pos,searchName,searchStartTagType,searchStartTagType==StartTagType.NORMAL ? Tag.isXMLName(searchName) : true);
	}

	static StartTag findNext(final Source source, final int pos, final String searchName, final StartTagType searchStartTagType, final boolean isXMLTagName) {
		...
		if (searchName==null) return (StartTag)source.findNextTag(pos,searchStartTagType);
		final char[] startDelimiterCharArray=getStartDelimiterCharArray(searchName);
		try {
			final ParseText parseText=source.getParseText();
			int begin=pos;
	...
	static StartTag findPrevious(final Source source, int pos) {
		while (true) {
			final Tag tag=Tag.findPreviousTag(source,pos);
	...
	static StartTag findNext(final Source source, int pos) {
		while (true) {
			final Tag tag=Tag.findNextTag(source,pos);
			if (tag==null) return null;
			if (tag instanceof StartTag) return (StartTag)tag;
	...
	static StartTag findNext(final Source source, final int pos, final String attributeName, final String value, final boolean valueCaseSensitive) {
		...
		while (searchPos<source.end) {
			...
			final Tag tag=source.findEnclosingTag(searchPos);
				...
				if (startTag.getAttributes()!=null) {
					...
					if (attributeValue!=null) {
						...
						if (value.equalsIgnoreCase(attributeValue)) {
							...
							if (source.logger.isInfoEnabled()) source.logger.info(source.getRowColumnVector(searchPos).appendTo(new StringBuffer(200)).append(": StartTag with attribute ").append(attributeName).append("=\"").append(attributeValue).append("\" ignored during search because its case does not match search value \"").append(value).append('"').toString());
						}
					}
				}
			}
	...
	private Segment[] findEndTag(final EndTag nextEndTag, final boolean checkForEmptyElementTag, final boolean isXMLTagName) {
		// assert nextEndTag!=null
		StartTag nextStartTag=findNext(source,end,name,startTagType,isXMLTagName);
		if (checkForEmptyElementTag) {
			while (nextStartTag!=null && nextStartTag.isSyntacticalEmptyElementTag())
				nextStartTag=findNext(source,nextStartTag.end,name,startTagType,isXMLTagName);
		}
		return findEndTag(end,nextStartTag,nextEndTag,checkForEmptyElementTag,isXMLTagName);
	}

	private Segment[] findEndTag(final int afterPos, final StartTag nextStartTag, final EndTag nextEndTag, final boolean checkForEmptyElementTag, final boolean isXMLTagName) {
		...
		final Segment[] findResult=nextStartTag.findEndTag(nextEndTag,checkForEmptyElementTag,isXMLTagName);  // find the matching end tag to the interloping start tag
		if (findResult==null) return null;  // no end tag in the rest of the file
		final EndTag nextStartTagsEndTag=(EndTag)findResult[0];
		final EndTag nextNextEndTag=EndTag.findNext(source,nextStartTagsEndTag.end,nextEndTag.getName(),nextEndTag.getEndTagType()); // find end tag after the interloping start tag's end tag
		return findEndTag(nextStartTagsEndTag.end,(StartTag)findResult[1],nextNextEndTag,checkForEmptyElementTag,isXMLTagName);  // recurse to see if this is the matching end tag
	}
}
