...
final class StartTagTypeUnregistered extends StartTagType {
	...
	protected Tag constructTagAt(final Source source, final int pos) {
		...
		if (source.logger.isInfoEnabled()) source.logger.info(source.getRowColumnVector(tag.getBegin()).appendTo(new StringBuffer(200).append("Encountered possible StartTag at ")).append(" whose content does not match a registered StartTagType").toString());
		return tag;
	}
}
